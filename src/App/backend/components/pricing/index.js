...
import Plans from './redux/actions';
import data from '../../../shared/helper/countries.json';
...
class Pricing extends Component {
	...
	render() {
		...
		async function getResults() {
			const html = await axios.get('http://localhost:5000/flags/alb.svg');
			const $ = await cheerio.load(html.data);
			console.log('fdfdfdfddfdfdff', $);

			// var namespaced = addNameSpace(`<?xml version="1.0" encoding="UTF-8"?>
			...
			// </svg>`);
			// namespaced = namespaced.replace(/"/g, "'");
			// namespaced = namespaced.replace(/>\s{1,}</g, '><');
			// namespaced = namespaced.replace(/\s{2,}/g, ' ');
			// namespaced = namespaced.replace(symbols, encodeURIComponent);
			// return `data:image/svg+xml,${namespaced}`;
		}
...
const mapDispatchToProps = dispatch => {
	return {
		getPlans: async () => dispatch(await Plans.getPlan()),
	};
};

const mapStateToProps = state => {
	return {
		payload: state.GetPlans.payload,
		pending: state.GetPlans.pending,
		error: state.GetPlans.error,
	};
};
