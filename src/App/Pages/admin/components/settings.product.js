				...
				<form className='h-100' onSubmit={handleSubmit}>
							...
							<div className='input-group flex-row-reverse'>
								<div className='input-group-append'>
									<span className='input-group-text cv-input-group-text rounded-0 bg-transparent p-0 text-secondary pl-2' id='basic-addon2'>
										{(location.currencies && location.currencies[0].name) || country.currencies[0].name}
									</span>
								</div>
								<div className='input-group-prepend'>
									<button
										...
										aria-expanded='false'
									>
										<img height='17' width='30' className='pr-1' src={`http://localhost:5000/flags/${location.flag || country.flag}`} alt='flag' />{' '}
										{location.currencies[0].symbol || country.currencies[0].symbol}
									</button>
									<div className='dropdown-menu country-select rounded-sm shadow-xs'>
										{countries.map((e, index) => {
											return (
												<a key={index} onClick={() => this.props.set.setState({ location: e })} className='dropdown-item d-flex align-items-center' href='#'>
					...
					<li className='d-flex justify-content-between py-3 border-bottom'>
						...
						<div className='custom-control custom-switch'>
							<input name='autoCurrency' checked={state.autoCurrency} type='checkbox' className='custom-control-input ml-auto' id='autoCurrency' onChange={handleInput} />
					...
					<li className='d-flex justify-content-between py-3 border-bottom'>
						...
						<div className='custom-control custom-switch'>
							<input name='currencyFlagDisplay' checked={state.currencyFlagDisplay} type='checkbox' className='custom-control-input ml-auto' id='currencyFlagDisplay' onChange={handleInput} />
					...
					<li className='d-flex justify-content-between align-items-center border-bottom py-3'>
						...
						<div className=''>
							<div className='custom-control custom-radio custom-control-inline'>
								<input type='radio' checked={state.currencyDisplayBy === 'code'} id='currencyDisplayBy-code' name='code' className='custom-control-input' onChange={handleInput} />
								<label className='custom-control-label' htmlFor='currencyDisplayBy-code'>
									code <small>( USD )</small>
								</label>
							</div>
							<div className='custom-control custom-radio custom-control-inline'>
								<input type='radio' checked={state.currencyDisplayBy === 'symbol'} id='currencyDisplayBy-symbol' name='code' className='custom-control-input' onChange={handleInput} />
					...
					<li className='d-flex justify-content-between py-3'>
						...
						<div className='custom-control custom-switch'>
							<input name='autoProductApproval' checked={state.autoProductApproval} type='checkbox' className='custom-control-input ml-auto' id='autoProductApproval' onChange={handleInput} />
