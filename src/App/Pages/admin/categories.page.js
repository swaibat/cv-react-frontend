...
class Categories extends Component {
	...
	render() {
		const { payload, deletePayload } = this.props;
		return (
			<>
				<Sidenav />
												...
												<div className='card border-0 shadow-sm'>
														...
														<ul class='list-group list-group-flush mt-2 hr-9 overflow-y-scroll text-secondary'>
																...
																return (
																	<li key={key} class='list-group-item list-group-item-sm border-0 d-flex align-items-center category-list text-capitalize'>
																		...
																		<div class='btn-group btn-group-sm ml-auto mr-1' role='group' aria-label='Basic example' onClick={this.handleClick}>
																			<button name={`${category.name}>${sub.name}`} id={sub.id} type='button' class='btn text-primary' data-toggle='modal' data-target='#updateModel' onClick={this.handleClick}>
																				<FontAwesomeIcon className='' icon={faEdit} />
																			</button>
																			<button name={`${category.name}>${sub.name}`} id={sub.id} type='button' class='btn text-danger' data-toggle='modal' data-target='.bd-example-modal-sm' onClick={this.handleClick}>
															...
															{this.state.name && category.id === parseInt(this.state.ParentId) ? (
																...
																<li class={`list-group-item list-group-item-sm border-0 text-center m-auto`}>
																	...
																	<button name={category.name} id={category.id} className='btn btn-sm btn-outline-primary' data-toggle='modal' data-target='#exampleModalCenter' onClick={this.handleClick}>
																		<FontAwesomeIcon className='mr-2' icon={faPlus} />
																		add one
																	</button>
																</li>
													...
													<div className='card-footer d-flex p-2 align-items-center shadow-sm-top bg-white text-secondary'>
														...
														<div class='btn-group btn-group-sm ml-auto' role='group' aria-label='Basic example'>
															<div class='btn-group btn-group-sm ml-auto mr-1' role='group' aria-label='Basic example' onClick={this.handleClick}>
																<button name={`${category.name}>${''}`} id={category.id} type='button' class='btn text-primary' data-toggle='modal' data-target='#updateModel' onClick={this.handleClick}>
																	<FontAwesomeIcon className='' icon={faEdit} />
																</button>
																<button name={`${category.name}>${''}`} id={category.id} type='button' class='btn text-danger' data-toggle='modal' data-target='.bd-example-modal-sm' onClick={this.handleClick}>
																	<FontAwesomeIcon icon={faTrash} />
																</button>
															</div>
														</div>
