...
function BulkDelete({ data }) {
	return (
		<div className='modal fade' id='bulkDelete' tabindex='-1' role='dialog' aria-labelledby='bulkDeleteLabel' aria-hidden='true'>
						...
						<ul>
							{data.state.bulk.map((cat, i) => (
								<li key={i} className='w-100 ml-n2 d-flex align-items-center'>
									...
									{cat.split('>')[0]} <small className='text-secondary font-weight-light'> ({cat.split('>')[2]})</small>
